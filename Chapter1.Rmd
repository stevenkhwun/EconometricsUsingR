...
2.  The second key assumption is on the expected value of $u$ given $x$ or, in other words, on the mean of the error term for each slice of the population determined by the values of $x$, that is:
\begin{equation}
...
We use the **least squares** criterion to choose $\hat{\beta_{0}}$ amd $\hat{\beta_{1}}$ so as to minimize _**SSR**_, the **residual sum of squares** (sum of the squares of the residuals).

$$SSR=\sum_{i=1}^{n}{\hat{u}_{i}}^2$$
This is usually referred to as **ordinary least squares** and abbreviated _**OLS**_.  
<br/>
The square of the residual in observation $i$ in terms of $\hat{\beta_{0}}$ and $\hat{\beta_{1}}$, and the data on $X$ and $Y$:


\begin{eqnarray}
{\hat{u}_{i}}^2 &=&(Y_i-\hat{Y}_{i})^2 =(Y_i-\hat{\beta}_{0}-\hat{\beta}_{1}X_i)^2 \\

&=& Y_i^2+\hat{\beta}_{0}^2+\hat{\beta}_{1}^2X_i^2-2\hat{\beta}_{0}Y_i-2\hat{\beta}_{1}X_iY_i+2\hat{\beta}_{0}\hat{\beta}_{1}X_i \\
\end{eqnarray}
...
Summing over all the $n$ observations, we can write _SSR_ as

$$SSR=\sum_{i=1}^{n}Y_i^2+n\hat{\beta}_{0}^2+\hat{\beta}_{1}^2\sum_{i=1}^{n}X_i^2-2\hat{\beta}_{0}\sum_{i=1}^{n}Y_i-2\hat{\beta}_{1}\sum_{i=1}^{n}X_iY_i+2\hat{\beta}_{0}\hat{\beta}_{1}\sum_{i=1}^{n}X_i$$

We find the particular values of $\hat{\beta}_{0}$ and $\hat{\beta}_{1}$ by minimize _SSR_. The partial differentials of _SSR_ with respect to $\hat{\beta}_{0}$ and $\hat{\beta}_{1}$ are:

$$\frac{\partial{SSR}}{\partial{\hat{\beta}_{0}}}=2n\hat{\beta}_{0}-2\sum_{i=1}^{n}Y_i+2\hat{\beta}_{1}\sum_{i=1}^{n}X_i$$

$$\frac{\partial{SSR}}{\partial{\hat{\beta}_{1}}}=2\hat{\beta}_{1}\sum_{i=1}^{n}X_i^2-2\sum_{i=1}^{n}X_iY_i+2\hat{\beta}_{0}\sum_{i=1}^{n}X_i$$

The values of $\hat{\beta}_{0}$ and $\hat{\beta}_{1}$ that minimize _RSS_ must satisfy the first-order conditions

\begin{equation}
\frac{\partial{SSR}}{\partial{\hat{\beta}_{0}}}=0
\quad\mathrm{and}\quad
\frac{\partial{SSR}}{\partial{\hat{\beta}_{1}}}=0
\end{equation}



Hence
$$2n\hat{\beta}_{0}-2\sum_{i=1}^{n}Y_i+2\hat{\beta}_{1}\sum_{i=1}^{n}X_i=0$$
$$2\hat{\beta}_{1}\sum_{i=1}^{n}X_i^2-2\sum_{i=1}^{n}X_iY_i+2\hat{\beta}_{0}\sum_{i=1}^{n}X_i=0$$
